...
public class TaskRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments var){
        executorService.execute(() -> {
            while (true) {
                try {
                    RedPacketMessage message = RedPacketQueue.getQueue().take();
                    if(message!=null){
                         ...
                         * 获取剩余红包个数以及金额
                         */
                        int num = (int) redisUtil.get(redPacketId+"-num");
                        int restMoney = (int) redisUtil.get(redPacketId+"-money");
                        LOGGER.info("剩余红包个数{}，剩余红包金额{}",num,restMoney);
                        /**
                         * 清空红包数据
                         */
                        redisUtil.remove(redPacketId+"-num");
                        redisUtil.remove(redPacketId+"-money");
                        /**
                         * 异步更新数据库、异步退回红包金额
                         */
                    }
